...
**เร่งรัดการอัพเดตสถานะ (Escalation, แยกจาก SLA):** เรื่องที่อยู่ระหว่างดำเนินการ (ASSIGNED/ACCEPTED/IN_PROGRESS) ไม่มีอัพเดตความคืบหน้า → แจ้งเตือน 3 ครั้ง: 30 วัน (center+agency_users) → 45 วัน (center+agency_users) → 52 วัน (center+agency_users) · นับจากอัพเดตล่าสุด/วันรับเรื่อง · มีอัพเดต → รีเซ็ต (ฟิลด์ `last_progress_at`/`escalation_level`; ดู `08` §4.3).
