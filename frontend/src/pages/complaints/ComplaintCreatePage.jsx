      ...
      if (pendingFiles.length && complaintId) {
        await Promise.all(
          pendingFiles.map((file) => {
            ...
            fd.append('file', file);
            return complaintApi.uploadAttachment(complaintId, fd).catch(() => {});
          })
        );
      }
  ...
  return (
    <Box>
      ...
      <Box display="flex" gap={2} mt={3}>
        <Button variant="outlined" onClick={() => navigate('/complaints')} disabled={loading}>
          ยกเลิก
        </Button>
        <Button
          variant="contained"
          ...
          disabled={loading || masterData.loading}
        >
          {loading ? 'กำลังบันทึก...' : 'บันทึกเรื่องร้องเรียน'}
        </Button>
      </Box>
    </Box>
  );
